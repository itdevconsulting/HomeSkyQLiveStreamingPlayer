...
Sky Stream remote is stricter: run the container on the UK-egress gateway box next to the puck. Keep the puck on that gateway. TCP 8091 does not work over a Tailscale subnet route that only forwards ping. Put Tailscale in front of the web UI, not between this container and the puck.
